...
async function findEpisodesExercises(client) {
  ...
  const result = await client.db("databaseWeek3").collection("bob_ross_episodes").find({episode: "S02E02"}).toArray();

  console.log(
    `The title of episode 2 in season 2 is ${result[0].title}`
  );

  // Find the season and episode number of the episode called "BLACK RIVER" [Should be: S02E06]
  const result1 = await client.db("databaseWeek3").collection("bob_ross_episodes").findOne({title: "BLACK RIVER"});
  ...
  const result2 = await client.db("databaseWeek3").collection("bob_ross_episodes").find({elements: "CLIFF"}).toArray();

console.log(
    `The episodes that Bob Ross painted a CLIFF are [${result2.map(element => element.title)}]`);
...
async function updateEpisodeExercises(client) {
  ...
  const result = await client.db("databaseWeek3").collection("bob_ross_episodes").updateOne({episode: "S30E13"}, {$set: {title:"BLUE RIDGE FALLS"}})
  ...
  const result1 = await client.db("databaseWeek3").collection("bob_ross_episodes").updateMany({elements: "BUSHES"}, {$set: {"elements.$[elem]": "BUSH"}}, {arrayFilters: [{elem: "BUSHES"}]})
...
async function deleteEpisodeExercise(client) {
   ...
   */
  const result = await client.db("databaseWeek3").collection("bob_ross_episodes").deleteOne({episode: "S31E14"});
  console.log(
